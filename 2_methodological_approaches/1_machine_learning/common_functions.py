...
def get_coords(LON, lonmin, lonmax, LAT,latmin, latmax, nearest_flag):
    ...
    '''
    dist_i1 = point_distance(latmin,LAT,lonmin,LON)
    #J is the X-coord
    ...
    if nearest_flag:
        ...
        dist_i2 = point_distance(latmax,LAT,lonmax,LON)
...
def subset_image(grid_lat, grid_lon, plot_extents):
    '''
     ...
     FULL-ORBIT or POLAR DATA. Uses point_distance 
    ...
    '''
    # bottom left
    dist = point_distance(plot_extents[2], grid_lat, plot_extents[0], grid_lon)
    i0, j0 = np.unravel_index(dist.argmin(), dist.shape)
    
    # bottom right
    dist = point_distance(plot_extents[2], grid_lat, plot_extents[1], grid_lon)
    i1, j1 = np.unravel_index(dist.argmin(), dist.shape)    
    
    # top right
    dist = point_distance(plot_extents[3], grid_lat, plot_extents[1], grid_lon)
    i2, j2 = np.unravel_index(dist.argmin(), dist.shape)
    
    # top left
    dist = point_distance(plot_extents[3], grid_lat, plot_extents[0], grid_lon)

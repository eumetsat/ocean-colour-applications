...
"""
Executable CLI script that takes a config file and an L2 input file.

Usage:
    ./run_l2_netcdf.py --config_template config.xml --input_dir S3????.SAFE --gpt_path /Applications/esa-snap/bin/gpt
"""
...
def prep_config(args):
    ...
    if fnmatch.fnmatch(os.path.basename(args.input_dir), "S3?_OL_2_W?R*.SEN3"):
        print("Activating Sentinel-3 product options")
        manifest = "xfdumanifest.xml"
    elif fnmatch.fnmatch(os.path.basename(args.input_dir), "S2?_MSIL2A*.SAFE"):
        print("Activating Sentinel-2 product options")
        manifest = "MTD_MSIL2A.xml"
    else:
        print("Input file does not match required S3/S2 L2 product")
        sys.exit()
